...
function respawn(tx, ty)
    ...
    if prop then
        ...
        if eid then
            local x, y = c_get_pos_in_zone(tx, ty, 2, 2)
            if x then
                local obj = {_id=eid, eid=eid, x=x, y=y, propid=prop.ID, size=prop.Size, born=gTime, val=prop.Count, pid=0, uid=0 }
                gEtys[ eid ] = obj
                etypipe.add(obj)
                checkin(obj)
            else
...
function do_check(zx, zy)
    if zx >= 0 and zx < 80 and zy >= 0 and zy < 80 then
        ...
        for k, eid in pairs(node or {})  do
            ...
            if ety then
                if not ety.on and ety.born < gTime - 12 * 3600 then
                    --todo
                    rem_ety(ety)
                else

...
--------------------------------------------------------------------------------
-- Function : 出战返回
-- Argument : hero_id, hp_ratio
-- Return   : succ - true; fail - false
-- Others   : NULL
--------------------------------------------------------------------------------
function back_from_battle(hero_id, hp_ratio)
    local hero = get_hero_by_uniq_id(hero_id)
    if not hero then
        ERROR("back_from_battle: get hero failed. hero_id = %s.", hero_id or "")
        return false
    end

    -- 血量修改
    hero:update_hp(hp_ratio)

    if hero.status == HERO_STATUS_TYPE.MOVING then
        hero.status = HERO_STATUS_TYPE.FREE
    end

    -- TODO: 城建中被拉出去打架的，打完没有被俘虏的，接着做之前的事

    if hero.prisoner ~= 0 then
        local ply = getPlayer(hero.pid)
        if ply then
            ply:imprison_hero(hero.prisoner)
            hero.prisoner = 0
        else
            ERROR("back_from_battle: get player failed. hero_id = %s, hero.pid = %d.", hero_id, hero.pid)
            return
        end
    end
...
function capture(src_hero_id, des_hero_id)
    ...
    des_hero.capturer = src_hero.pid

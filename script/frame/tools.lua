...
function svnnum( _svn_info )
    ...
    if key == nil then
        WARN("[SCRIPT](lua) regist_file_svn fail. catch filename fail. " .. (_svn_info or "") )
        return
    end
    if svn == nil then
        WARN("[SCRIPT](lua) regist_file_svn fail. catch svn fail. " .. (_svn_info or "") )
        return
    end
...
function doDumpTab(t, step)
    step = step or 0
    if type(t) == "table" then
        for k, v in pairs(t) do
            if type(v) == "table" then
                LOG("%s%s = {", mkSpace(step*2+2), tostring(k))
                doDumpTab(v, step+2)
                LOG("%s}", mkSpace(step*2+2))
            else
                LOG("%s%s = %s", mkSpace(step*2+2), tostring(k), tostring(v))
            end
        end
    else
        LOG("%s%s = %s", mkSpace(step*2), tostring(k), tostring(v))
    end
end

function dumpTab(t, what)

...
_example = {
    ...
    culture      = HERO_CULTURE_TYPE.CHINA,
...
function change_basic_skill(self, skill_idx, skill_id, exp)
    ...
    -- 更新 _ef_hero
    if skill_idx == 2 and old_skill_id ~= skill_id then
        local player = getPlayer(self.pid)
        if not player then
            ERROR("change_basic_skill: get player failed, pid = %d.", self.pid)
            return
        end

        player:update_ef_hero()
    end
end
...
function basic_skill_changed(self, skill_idx)
    ...
    if not skill then
        ...
        -- 入库
        local db = dbmng:getOne()
        db.hero:update({_id = self._id}, {["$set"] = {["basic_skill"] = self.basic_skill}})

        dumpTab(self.basic_skill, string.format("update hero[%s].basic_skill", self._id))

        -- 告知前端
        local player = getPlayer(self.pid)
        if player then
            return Rpc:on_basic_skill_changed(player, self.idx, skill_idx, skill[1], skill[2])
        else
            ERROR("basic_skill_changed: getPlayer(%d) failed.", self.pid or -1)
        end
    end
end
...
function can_def(self)
    ...
    if can_def_status[self.status] then
        return true
    else
        return false
    end

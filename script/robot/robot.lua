...
--lxz(gIdx,ncount)
        for i = 1, 10, 1 do--每秒登录人数
            ...
            gIdx = gIdx + 1
            local name = string.format("6666%d", gIdx)
        ...
        while true do
            ...
            if not tag then
                ...
                if mem > (gMaxMem or 0) then
                    gMaxMem = mem
                    --INFO("[LUA_MEM], maxlua = %d", gMaxMem / 1024)
                end
                ...
                if fname then
                    local n = gConns[ sid ]
                    if n then
                        lxz((n.pid or 0)..":"..fname..":"..gFrame,args)
                        local f = Ply[fname ]
                        if f then
                            gActive[ sid ] = n
                            f(n, unpack(args) )
    ...
    local as = gActive
    gActive = {}
    for k, n in pairs(as) do
        if n.action and #n.action > 0 then
            Ply.doAction(n)
            n.tmAction = gTime
        end
    end

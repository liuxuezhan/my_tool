...
if(!defined('_LOGGER_PHP_')) {
    ...
    class Logger {
        ...
        function _log($level, $message, $name) {
            ...
            $content = date('Y-m-d H:i:s') . ' [' . $log_level_name . '] ';
        ...
        function debug($message, $name = LOG_FILE) {
            $this->_log(LEVEL_DEBUG, $message, $name);
        }
        function info($message, $name = LOG_FILE) {
            $this->_log(LEVEL_INFO, $message, $name);
        }
        function warn($message, $name = LOG_FILE) {
            $this->_log(LEVEL_WARN, $message, $name);
        }
        function error($message, $name = LOG_FILE) {
            $this->_log(LEVEL_ERROR, $message, $name);
        }
        function fatal($message, $name = LOG_FILE) {

...
class SnsNetwork
{
	 ...
	 * @return array 结果数组
	 */
	static public function makeRequest($url, $params, $cookie, $method='post', $protocol='http', $logger = null, $open_id = null)
	{
		$query_string = self::makeQueryString($params);	   
	    $cookie_string = self::makeCookieString($cookie);

	    if($logger !== null){
	        $logger->debug("makeRequest query_string: {$query_string}");
	        $logger->debug("makeRequest cookie_string: {$cookie_string}");
	    }
        ...
        $X = $E - $B;
        if ($logger != null and $open_id != null and $X > 0.3 ) {
            $logger->debug("open_id: {$opne_id} makeRequest curl_exec delay call: {$X} sec, ");
        }
